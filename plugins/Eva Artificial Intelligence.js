...
if (conf.LANG == 'EN') vtalk_dsc = 'Activate Luttappi Chat Bot.', reply_eva = '*reply to any message send by the bot!*'
...
if (conf.LANG == 'ML') vtalk_dsc = 'Activate Luttappi Chat Bot.', reply_eva = '*reply to any message send by the bot!*'
...
Asena.addCommand({on: 'text', fromMe: wk, dontAddCommandList: true, deleteCommand: false}, (async (message, match) => {
    if (message.message.startsWith('bot') && conf.FULLEVA !== 'true') {        
        ...
        await axios.get('http://api.brainshop.ai/get?bid=161968&key=h1ZIwk4MMM3MwC6s&uid=' + unique_ident + '&msg=' + uren).then(async (response) => {
            var fins = ''                           
            if (conf.LANG !== 'EN') {
    ...
    try {
        ...
        convertToWav(file)
            .on('end', async () => {
                ...
                await axios.get('http://api.brainshop.ai/get?bid=161968&key=h1ZIwk4MMM3MwC6s&uid=' + unique_ident + '&msg=' + son).then(async (response) => {
                    var trmsg = ''
